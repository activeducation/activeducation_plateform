...
class TestsAdminRepository:
    ...
    async def list_tests(
        ...
        for t in result.data or []:
            # Count questions
            qc = (
                self._db.client.table("test_questions")
    ...
    async def get_test_detail(self, test_id: UUID) -> TestDetail:
        test = self._db.fetch_one(table="orientation_tests", id_column="id", id_value=str(test_id))
        if not test:
        ...
        for q in questions_data.data or []:
            options = [OptionSummary(**o) for o in (q.get("question_options") or [])]
