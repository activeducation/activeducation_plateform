...
class SchoolCourseEditorPage extends StatefulWidget {
  final String? courseId;
  const SchoolCourseEditorPage({super.key, this.courseId});

  @override
  State<SchoolCourseEditorPage> createState() => _SchoolCourseEditorPageState();
}

class _SchoolCourseEditorPageState extends State<SchoolCourseEditorPage> {
  ...
  Future<void> _loadCourse() async {
    ...
    try {
      ...
      final response = await api.get(ApiEndpoints.schoolCourseById(widget.courseId!));
  ...
  Future<void> _saveCourse() async {
    ...
    try {
      ...
      if (_isEditing) {
        ...
        final response = await api.post(ApiEndpoints.schoolCourses, data: data);
        if (mounted) {
          AdminSnackbar.success(context, 'Cours créé');
          context.go('/school-portal/courses/${response.data['id']}/edit');
        }
        return;
      }
    } catch (e) {
  ...
  Future<void> _togglePublish() async {
    try {
      final api = getIt<ApiClient>();
      await api.patch(ApiEndpoints.schoolCoursePublish(widget.courseId!), data: {'is_published': !_isPublished});
  ...
  Future<void> _addModule() async {
    ...
    try {
      final api = getIt<ApiClient>();
      final response = await api.post(ApiEndpoints.schoolCourseModules(widget.courseId!), data: {'title': titleCtrl.text, 'description': descCtrl.text.isNotEmpty ? descCtrl.text : null, 'display_order': _modules.length});
  ...
  Future<void> _deleteModule(String moduleId) async {
    try {
      final api = getIt<ApiClient>();
      await api.delete(ApiEndpoints.schoolModule(moduleId));
  ...
  Future<void> _addLesson(String moduleId) async {
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          content: SizedBox(width: 400, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const SizedBox(height: 12),
            if (lessonType == 'text') TextField(controller: contentCtrl, decoration: const InputDecoration(labelText: 'Contenu'), maxLines: 4),
            if (lessonType == 'video') TextField(controller: videoCtrl, decoration: const InputDecoration(labelText: 'URL vidéo')),
          ]))),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Annuler')),
            ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Ajouter')),
    ...
    try {
      final api = getIt<ApiClient>();
      final data = {'title': titleCtrl.text, 'lesson_type': lessonType, 'display_order': 0};
      if (lessonType == 'text' && contentCtrl.text.isNotEmpty) data['content'] = contentCtrl.text;
      if (lessonType == 'video' && videoCtrl.text.isNotEmpty) data['video_url'] = videoCtrl.text;

      await api.post(ApiEndpoints.schoolModuleLessons(moduleId), data: data);
      await _loadCourse();
  ...
  Future<void> _deleteLesson(String lessonId) async {
    try {
      final api = getIt<ApiClient>();
      await api.delete(ApiEndpoints.schoolLesson(lessonId));
      await _loadCourse();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(icon: const Icon(Icons.arrow_back), onPressed: () => context.go('/school-portal/courses')),
        actions: [
          if (_isEditing)
            TextButton.icon(
